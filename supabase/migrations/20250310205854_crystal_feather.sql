-- Створення користувачів та профілів
...
BEGIN
  FOR i IN 1..10 LOOP
    ...
    INSERT INTO profiles (
      ...
      NOW() -- Записати час оновлення
    ) RETURNING id INTO profile_id;

    -- Створення інвесторського профілю
    INSERT INTO investor_profiles (id)
    VALUES (profile_id);
  END LOOP;
END $$;

-- Створення рецензій для синдикаторів

-- Додати рецензії для Summit Capital Partners
INSERT INTO syndicator_reviews (id, syndicator_id, reviewer_id, rating, review_text, created_at)
SELECT
  gen_random_uuid(),
...
-- Додати рецензії для Horizon Investment Group
INSERT INTO syndicator_reviews (id, syndicator_id, reviewer_id, rating, review_text, created_at)
SELECT
  gen_random_uuid(),
...
-- Додати рецензії для Metropolitan Real Estate
INSERT INTO syndicator_reviews (id, syndicator_id, reviewer_id, rating, review_text, created_at)
SELECT
  gen_random_uuid(),
...
-- Додати рецензії для Innovation Healthcare Properties
INSERT INTO syndicator_reviews (id, syndicator_id, reviewer_id, rating, review_text, created_at)
SELECT
  gen_random_uuid(),
...
-- Додати рецензії для Evergreen Residential
INSERT INTO syndicator_reviews (id, syndicator_id, reviewer_id, rating, review_text, created_at)
SELECT
  gen_random_uuid(),
