...
import { 
  ...
  MapPin, CheckCircle, ArrowUpRight, ArrowRight
} from 'lucide-react';
  ...
  async function fetchDashboardData() {
    try {
      const [
        ...
        supabase.from('deal_interests').select('*', { count: 'exact', head: true }).eq('investor_id', user!.id),
      ...
      const { data: recentDealsData } = await supabase
        .from('deals')
        .select(`*, syndicators (company_name, company_logo_url, verification_status)`)
        .eq('status', 'active')
      ...
      const { data: favoritesData } = await supabase
        .from('favorites')
        .select(`deals (*, syndicators (company_name, verification_status))`)
  ...
  return (
    <div className="space-y-6">
      ...
      {stats.messagesUnread > 0 && (
        <Link 
          to="/inbox"
          className="block bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl p-5 hover:shadow-lg transition-all"
        >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="font-bold">
                  {stats.messagesUnread} unread message{stats.messagesUnread !== 1 ? 's' : ''}
                </h3>
                <p className="text-blue-200 text-sm">Syndicators have responded to your inquiries</p>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          {favoriteDeals.length === 0 ? (
            <div className="p-6 text-center">
              ...
              <Link to="/find" className="text-blue-600 hover:underline text-xs">Start exploring</Link>
            </div>
          ) : (
            <div className="divide-y divide-gray-100">
