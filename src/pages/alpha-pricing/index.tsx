...
import { Building2, Users, Mail, CheckCircle, DollarSign, Star, ArrowRight, Zap, Shield, Target, TrendingUp, Eye, BarChart, RefreshCw, ChevronDown, ChevronUp, Crown } from 'lucide-react';
...
const pricingTiers = [
  {
    name: 'Free',
    tagline: 'Get Started in Our Directory',
    monthlyPrice: 0,
    annualPrice: 0,
    trialDays: 0,
    dealPosts: 0,
    ...
    refundPolicy: 'Upgrade prompted after 14 days or 50 views',
    ctaText: 'Join Free',
...
export function AlphaPricing() {
  const { user } = useAuthStore();
  const [showAuthModal, setShowAuthModal] = useState(false);
  const [billingInterval, setBillingInterval] = useState<'monthly' | 'annual'>('monthly');
  const [isSubscribing, setIsSubscribing] = useState(false);
  const [openCards, setOpenCards] = useState<number[]>([0, 1, 2, 3]); // Default to all open on mobile
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <SEO 
        title="Free Syndicator Plans | EquityMD Pricing"
        description="Join EquityMD's free syndicator plan or choose paid tiers with trials for 10K+ accredited investors."
        keywords="free syndicator plan, CRE deals, free trial, commercial real estate pricing, syndication platform, guaranteed results"
        canonical="https://equitymd.com/alpha-pricing"
      />
      <Navbar />
      ...
      <div className="bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 text-white py-20 lg:py-32">
        <div className="max-w-6xl mx-auto text-center px-4">
          <h1 className="text-4xl lg:text-6xl font-bold mb-6">
            Join the Directory Free or Grow with Our Plans
          </h1>
          <p className="text-xl lg:text-2xl text-blue-100 mb-8 max-w-3xl mx-auto">
            Risk-Free Start, Results Guaranteed
          </p>
          <p className="text-lg text-blue-200 max-w-2xl mx-auto mb-12">
            Start with a free directory listing and upgrade when you're ready to post deals. 
      ...
      <div className="max-w-7xl mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16 pt-4">
          {pricingTiers.map((tier, index) => (
            <div 
              ...
              } ${tier.isFree ? 'ring-2 ring-green-500' : ''}`}
            >
              {tier.popular && (
              ...
              {tier.isFree && (
                <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-10">
                  <div className="bg-green-600 text-white px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap shadow-lg">
                    Free Start
                  </div>
                </div>
              )}
              ...
              <div className="p-6 text-center">
                ...
                <div className="space-y-2 mb-4">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-medium">{tier.dealPosts === 0 ? 'None' : `${tier.dealPosts}/year`}</span>
                  </div>
              ...
              <div className="p-6 bg-gray-50 border-t border-gray-200">
                ...
                <p className="text-xs text-gray-500 text-center mt-2">
                  {tier.isFree ? 'No credit card required' : 'No credit card required, cancel anytime'}
                </p>
                {tier.isFree && (
                  <p className="text-xs text-blue-600 text-center mt-1 font-medium">
                    Upgrade prompted after 14 days or 50 views
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="mt-16 bg-gradient-to-r from-blue-50 to-green-50 rounded-lg p-8 text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">
            Start Free, Unlock Deals When Ready
          </h2>
          <p className="text-gray-600 mb-6 max-w-2xl mx-auto">
            Get listed in our directory for free and build your presence. When you're ready to post deals and access advanced features, 
            upgrade to Starter with a 14-day trial. No commitment required.
          </p>
        ...
        <div className="mt-20 max-w-3xl mx-auto">
          ...
          <div className="space-y-8">
            <div className="bg-white rounded-lg shadow-md p-6">
              <h3 className="font-bold text-lg mb-3">How does the free plan work?</h3>
              <p className="text-gray-600">
                Join our directory for free with a basic profile. After manual approval, you'll be visible to 10K+ investors. 
                After 14 days or 50 profile views, we'll offer you a chance to upgrade to post deals with our Starter plan.
              </p>
            </div>
            <div className="bg-white rounded-lg shadow-md p-6">
              <h3 className="font-bold text-lg mb-3">What's your results guarantee?</h3>
              <p className="text-gray-600">
                We stand behind our platform. Each paid tier includes specific guarantees: Starter offers $25 refund if no views, 
      ...
      <div className="md:hidden fixed bottom-4 left-4 right-4 z-50">
        <div className="flex gap-2">
          <a 
            href="/auth/signup?plan=free"
            className="flex-1 bg-green-600 text-white text-center py-4 rounded-lg shadow-lg font-medium hover:bg-green-700 transition-colors"
          >
            Join Free
          </a>
          <a 
            href="/auth/signup?plan=starter"
            className="flex-1 bg-blue-600 text-white text-center py-4 rounded-lg shadow-lg font-medium hover:bg-blue-700 transition-colors"
          >
            Start Trial
          </a>
        </div>
