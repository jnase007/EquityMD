...
export default function App() {
  ...
  // Aggressive session recovery for iOS/mobile (handles screen lock/unlock, tab suspension)
  useEffect(() => {
    const handleResume = async () => {
      if (document.visibilityState !== 'visible') return;

      authLogger.log('Tab resumed - checking session...');

      const tryRefresh = async (attempt = 1): Promise<void> => {
        try {
          const { data: { session } } = await supabase.auth.getSession();
          if (session) {
            authLogger.log('Session valid on attempt', attempt);
            // If we have a session but no user in store, restore it
          ...
          authLogger.log('Session null - refreshing (attempt', attempt, ')');
          const { error } = await supabase.auth.refreshSession();
          if (error) {
            authLogger.log('Refresh error:', error.message);
          } else {
            const { data: { session: refreshedSession } } = await supabase.auth.getSession();
            if (refreshedSession?.user) {
              authLogger.log('Session recovered on attempt', attempt);
          ...
          if (attempt < 3) {
            ...
            authLogger.log('Session recovery failed after retries');
          }
        } catch (err) {
          authLogger.log('Resume error:', err);
        }
      };
    ...
    // Listen for visibility changes, focus, and pageshow (bfcache)
  ...
  // Periodic heartbeat to keep session alive (every 5 minutes when visible)
  useEffect(() => {
    const heartbeat = setInterval(async () => {
      if (document.visibilityState === 'visible' && user) {
        const { data } = await supabase.auth.getSession();
        if (!data.session) {
          authLogger.log('Heartbeat: session lost, refreshing...');
